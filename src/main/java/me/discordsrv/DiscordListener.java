...
public void onMessageReceived(MessageReceivedEvent event) {
    ...
    String ipCommand = plugin.getConfig().getString("ip.command", "!ip");
    String onlineCommand = plugin.getConfig().getString("online.command", "!online");

    if (plugin.getConfig().getBoolean("ip.enabled", true)
    ...
    if (plugin.getConfig().getBoolean("online.enabled", true)
...
private void sendEmbed(MessageReceivedEvent event, String type) {
    ...
    String description = plugin.replacePlaceholders(
            plugin.getConfig().getString(path + "description", "")
    );

    String colorText = plugin.replacePlaceholders(
            plugin.getConfig().getString(path + "color", "#00AAFF")
    );
    ...
    List<Map<?, ?>> fields =
            plugin.getConfig().getMapList(path + "fields");
    ...
    for (Map<?, ?> field : fields) {
        ...
        embed.addField(
                plugin.replacePlaceholders(name),
                plugin.replacePlaceholders(value),
                inline
        );
    }

    String footer = plugin.getConfig().getString(path + "footer", "");
